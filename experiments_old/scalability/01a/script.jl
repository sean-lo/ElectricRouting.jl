    ...
    if write_log
        ...
        records = [
            (
                ...
                use_SR3_cuts = true,
                use_lmSR3_cuts = use_lmSR3_cuts,
                use_sparse_graph = use_sparse_graph,
                # Time taken
                ...
                LP_artificial_last = all_params[ind]["CGLP_artificial"],
                LP_artificial_last_SR3 = all_params[end]["CGLP_artificial"],
                ...
                IP_artificial_last = all_params[ind]["CGIP_artificial"],
                IP_artificial_last_SR3 = all_params[end]["CGIP_artificial"],
...
undone_indexes = setdiff(1:nrow(args_df), 
    [
        parse(Int, match(r"[\d]+(?=.csv)", x).match)
        for x in glob("results/*.csv", "$(@__DIR__)")
    ]
)
