...
function enumerate_single_customer_subpaths(
    starting_node::Int,
    ending_node::Int,
    customer::Int,
    data,
    ...
    j = ending_node
    p = customer
    d = customer + data["n_customers"]
    t = data["t"]
    α = data["α"]
    β = data["β"]
    
    arcs = [(i,p),(p,d),(d,j)]
    charge_required = sum(data["q"][a...] for a in arcs)
    ...
    elseif starting_node in data["N_charging"]
        T_range_here = T_range
        B_range_here = B_range
    else
        error()
    end

    time_bp = max(β[p] - t[i, p], 0.0)
    # time_1 = min(
    #     max(α[p] - t[i, p], 0.0),
    # )
    # from t in [0, time_1]:        
        # reach p at time:      t + t[i,p]
        # leave p at time:      α[p]
        # reach d at time:      α[p] + t[p,d]
        # leave d at time:      α[d]
        # reach j at time:      α[d] + t[d,j]
    time_2 = min(
        time_bp,
        max(α[d] - t[i, p] - t[p, d], 0.0),
    )
    # from t in [time_1, time_2]:
        # reach p at time:      t + t[i,p]
        # leave p at time:      t + t[i,p]
        # reach d at time:      t + t[i,p] + t[p,d]
        # leave d at time:      α[d]
        # reach j at time:      α[d] + t[d,j]
    time_3 = min(
        time_bp,
        max(β[d] - t[i, p] - t[p, d], 0.0),
        max(β[j] - t[i, p] - t[p, d] - t[d, j], 0.0)
    )
    # from t in [time_2, time_3]:
        # reach p at time:      t + t[i,p]
        # leave p at time:      t + t[i,p]
        # reach d at time:      t + t[i,p] + t[p,d]
        # leave d at time:      t + t[i,p] + t[p,d]
        # reach j at time:      t + t[i,p] + t[p,d] + t[d,j]
    for time in T_range_here
        if 0.0 ≤ time < time_2
            time_reach_j = α[d] + t[d, j]
        elseif time_2 ≤ time ≤ time_3  
            time_reach_j = time + t[i,p] + t[p,d] + t[d,j]
        else
            continue
        end
        append!(subpaths,
            [
                Subpath(
                    ...
                    arcs = copy(arcs),
                    time = time_reach_j,
                    charge = charge - charge_required,
                    served = copy(served),
                )
                for charge in B_range_here 
                if charge - charge_required ≥ 0
            ]
        )
    end
    return subpaths
...
function enumerate_single_customer_subpaths_withcharge(
    starting_node::Int,
    ending_node::Int,
    customer::Int,
    data,
    ...
    subpaths = enumerate_single_customer_subpaths(
        starting_node,
        ending_node,
        customer,
        data,
    ...
    for s in subpaths
        if s.current_node in data["N_depots"]
            s.round_time = dceil(s.end_time, T_range)
            s.round_charge = dfloor(s.end_charge, T_range)
            push!(subpaths_withcharge, s)
        else
            append!(subpaths_withcharge,
                [
                    Subpath(
                        n_customers = data["n_customers"],
                        starting_node = s.starting_node,
    ...
    return subpaths_withcharge
end


function enumerate_all_single_customer_subpaths(
    data, 
    T_range,
    B_range,
)
    subpaths_withcharge = []
    for (customer, i, j) in Iterators.product(
        data["N_pickups"],
        union(data["N_charging"], data["N_depots"]),
        union(data["N_charging"], data["N_depots"]),       
    )
        append!(subpaths_withcharge, 
            enumerate_single_customer_subpaths_withcharge(
                i, j, customer, data,
                T_range, B_range,
            )
        )
