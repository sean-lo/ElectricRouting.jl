...
begin 
    ...
    for n_customers in n_customers_range
        data = (
            ...
            |> x -> select(
                x, 
                [:seed, :time_taken_total_SR3, :LP_IP_gap_last_SR3]
            )
            |> x -> sort(
                x, 
                [order(:LP_IP_gap_last_SR3), order(:time_taken_total_SR3)]
            )
        )
        data.time_taken_total_SR3[data.LP_IP_gap_last_SR3 .> 0.0] .= 4000.0
        push!(time_ecdfs, CairoMakie.ecdfplot!(ax1, data.time_taken_total_SR3, xscale = log10))
        push!(gap_ecdfs, CairoMakie.ecdfplot!(ax2, data.LP_IP_gap_last_SR3))
    end
    axislegend(ax2, gap_ecdfs, ["$n_customers customers" for n_customers in n_customers_range], position = :rb)
    save("$(@__DIR__)/plots/ecdf.pdf", f)
    save("$(@__DIR__)/plots/ecdf.png", f)
end