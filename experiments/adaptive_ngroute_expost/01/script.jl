...
function run_instance(
    ...
    final_neighborhood = CG_all_neighborhoods[end]
    run_expost = @timed @suppress path_formulation_column_generation_with_adaptve_ngroute_SR3_cuts(
        data, graph,
        ;
        Env = GRB_ENV,
        method = method,
        ...
        ngroute = true,
        ngroute_alt = true,
        neighborhoods = final_neighborhood,
        verbose = true,
        use_smaller_graph = false,
        use_adaptive_ngroute = true,
        use_SR3_cuts = false,
    ...
    (
        CGLP_all_results_expost, CGIP_all_results_expost, CG_all_params_expost, CG_all_neighborhoods_expost, 
        all_params_expost, printlist_expost,
        some_paths_expost, model_expost, z_expost, SR3_constraints_expost
    ) = run_expost.value;
    all_params_expost_df = DataFrame(all_params_expost)
    ...
    records = [
        (
            ...
            time_taken_first_expost = all_params_expost_df[1, :CG_time_taken],
            time_taken_total = run.time,
            time_taken_total_expost = run_expost.time,
            sp_time_taken_mean_first = CG_all_params[1]["sp_time_taken_mean"],
            sp_time_taken_mean_first_expost = CG_all_params_expost[1]["sp_time_taken_mean"],
            sp_time_taken_mean_last = CG_all_params[end]["sp_time_taken_mean"],
            sp_time_taken_mean_last_expost = CG_all_params_expost[end]["sp_time_taken_mean"],
            # n_iterations
            converged = all_params_df[end, :converged],
            converged_expost = all_params_expost_df[end, :converged],
            time_limit = time_limit,
            time_limit_reached = all_params_df[end, :time_limit_reached],
            time_limit_reached_expost = all_params_expost_df[end, :time_limit_reached],
            n_iterations = length(CG_all_params),
            n_iterations_expost = length(CG_all_params_expost),
            n_CG_iterations = sum(CG_params["counter"] for CG_params in CG_all_params),
            n_CG_iterations_expost = sum(CG_params["counter"] for CG_params in CG_all_params_expost),
            # Objective values and gaps
            LP_objective_first = all_params[1]["CGLP_objective"],
            LP_objective_first_expost = all_params_expost[1]["CGLP_objective"],
            LP_objective_last = all_params[end]["CGLP_objective"],
            LP_objective_last_expost = all_params_expost[end]["CGLP_objective"],
            IP_objective_first = all_params[1]["CGIP_objective"],
            IP_objective_first_expost = all_params_expost[1]["CGIP_objective"],
            IP_objective_last = all_params[end]["CGIP_objective"],
            IP_objective_last_expost = all_params_expost[end]["CGIP_objective"],
            LP_IP_gap_first = all_params[1]["CG_LP_IP_gap"],
            LP_IP_gap_first_expost = all_params_expost[1]["CG_LP_IP_gap"],
            LP_IP_gap_last = all_params[end]["CG_LP_IP_gap"],
            LP_IP_gap_last_expost = all_params_expost[end]["CG_LP_IP_gap"],
            # neighborhood size
            neighborhood_size_mean_first = all_params[1]["ngroute_neighborhood_size"],
            neighborhood_size_mean_first_expost = all_params_expost[1]["ngroute_neighborhood_size"],
            neighborhood_size_mean_last = all_params[end]["ngroute_neighborhood_size"],
            neighborhood_size_mean_last_expost = all_params_expost[end]["ngroute_neighborhood_size"],
        )
    ]
    if write_log
    ...
    if write_results
        ...
        CSV.write("$(@__DIR__)/results/expost_$(row_index).csv", all_params_expost_df)    
    end
